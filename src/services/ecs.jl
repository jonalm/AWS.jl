...
- `name`: The resource name for which to modify the account setting. If
  ...
  affected.  When you specify fargateFIPSMode for the name and enabled for the value, Fargate
  uses FIPS-140 compliant cryptographic algorithms on your tasks. For more information about
  FIPS-140 compliance with Fargate, see  Amazon Web Services Fargate Federal Information
  Processing Standard (FIPS) 140-2 compliance in the Amazon Elastic Container Service
  Developer Guide.
- `value`: The account setting value for the specified principal ARN. Accepted values are
  enabled and disabled.
...
- `"ephemeralStorage"`: The amount of ephemeral storage to allocate for the task. This
  ...
  task requires the following platforms:   Linux platform version 1.4.0 or later.
